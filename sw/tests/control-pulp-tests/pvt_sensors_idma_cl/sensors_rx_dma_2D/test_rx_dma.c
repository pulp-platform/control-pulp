...
void probe_range(uintptr_t from, uintptr_t to, unsigned int samples)
{
	...
	if (((unsigned int)get_core_id()) < NUM_CORES) {
		...
		addr = from + incr * ((uintptr_t)core_samples) *
	...
	if (((unsigned int)get_core_id()) < NUM_CORES) {
		...
		//
		//		// get_time0 = get_time();
		//		mchan_transfer(core_samples * 4, RX, INC, TWD,
		// LIN, 1, 0, 0,
		...
		//		mchan_free(id);
	        ...
	        unsigned int dma_rx_id = pulp_cl_idma_memcpy_2d(
			(unsigned int)(tcdm_addr), ((unsigned int)addr), 4,
			(unsigned short int)4, (unsigned short int)incr,
			(unsigned short int)core_samples * 4);
		plp_cl_dma_wait(dma_rx_id);
	}
	...
	if (((unsigned int)get_core_id()) == 0) {
		...
		// printf("Mchan programming execution time = %d\n",
		// get_time1-get_time0);
	}
}
