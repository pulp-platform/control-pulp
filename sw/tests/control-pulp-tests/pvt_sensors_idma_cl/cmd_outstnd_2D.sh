...
run_test () {
    ...
    echo "TEST $1 $2: $3 outstanding transactions"
    echo "TEST $1 $2: $3 outstanding transactions" >&$5
...
do
  cd ../../../rtl/includes
  sed -i "s/\`define DMA_QUEUE_DEPTH  [0-9]*/\`define DMA_QUEUE_DEPTH  8/" pulp_soc_defines.sv
  sed -i "s/\`define NB_OUTSND_BURSTS [0-9]*/\`define NB_OUTSND_BURSTS ${j}/" pulp_soc_defines.sv
  cd ../../sim
  make clean all
  cd ../tests/control-pulp-tests/sensors_transfers/sensors_rx_dma_2D
  run_test DMA 2D $j 2 ${id1}
  cd ..
done
